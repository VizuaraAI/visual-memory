...
answer these three questions for Zamba2-VL-7B, whose 81 decoder layers all
carry a Mamba2 mixer while 13 also carry attention. First, just after reading
an image the state holds the gist, not the details: a linear probe reads which
object was present at $92.5\%$ on the best layer against $12.5\%$ chance, but
can barely read how many objects there were or where they sat. Second, how
fast the state forgets can be computed from the weights alone: each head has a
half-life of $\ln 2/(|A|\bar{\Delta})$ tokens, with medians of $6.6$, $2.9$
and $2.6$ at three scales, nothing fitted. Third, and centrally, the memory a
probe appears to find at distance is mostly not memory. With write amplitude
held fixed, heads with a $2.3$-token half-life still decode identity at
$26.6\%$ sixty-four tokens after the image, where their own parameters retain
$4\times10^{-9}$ of what was written. Evicting the image's key-value entries,
without touching the recurrent state, drops the fast tercile from $51.6\%$ to
$7.8\%$ at thirty-two tokens, below chance, while evicting an equal amount of
pre-image text moves the readout the other way; at matched amplitude, eviction
costs fast heads $2.4$ times what it costs slow ones. Causally, exchanging all
81 recurrent states between runs that saw different images leaves $100$ of
$100$ answers following the host's own image; exchanging seven of thirteen
attention caches moves $94$ to the donor's. The recurrent channel relays what
attention holds; it does not store the picture. The redundancy premise behind
visual token pruning survives down to a quarter of the entries, but the cost
of pruning harder roughly doubles with distance from the image.
\end{abstract}
...
$458{,}752$ numbers per layer across 81 layers, room enough to invite the
intuition that a few hundred image tokens fit comfortably inside. This paper
measures what is actually there. Our central finding is that the recurrent
state is not a memory of the image but a relay of it: at distance, what a
probe recovers is what the attention channel still holds and keeps
re-supplying. The evidence is causal, and the direction was not the one we
...
Mamba2 multiplies its state by $\exp(\Delta_t A)$ at every step, so a head's
half-life is $\ln 2/(|A|\bar{\Delta})$; the median is $6.6$, $2.9$ and $2.6$
tokens at three scales (Section~\ref{sec:horizon}). We also report where this
prediction fails.
...
their own predicted half-life are ordered as decay predicts, yet decode
...
other way, a double dissociation that rules out mere damage
(Section~\ref{sec:relay}).
...
\paragraph{Why this matters.} Anyone deploying a hybrid must decide how much
work the fixed state can be trusted with. This paper turns that judgement into
a calculation: from a checkpoint's decay weight $A$ and its gate statistics,
the retention horizon can be computed before any experiment is run, and for
this model it is single digits. The practical stake is visual token and
key-value pruning \citep{chen2024fastv,yang2024visionzip,shang2025prumerge,zhang2023h2o,li2024snapkv,xiao2024streaming},
which rests on the premise that most entries are redundant. The premise
largely survives on this hybrid; the safety margin does not. The redundancy
exists because the recurrent channel is relaying the picture, and it only
relays for a few tokens, so a method tuned on short prompts looks safer than
it is.
...
What is new here is therefore not that a fixed state forgets, but when it
forgets (a horizon computed rather than fitted), what survives (identity, not
count or position), where the answer actually comes from (a causal
localisation inside one model), and what that implies for a deployed
technique. Table~\ref{tab:novelty} in the appendix sets each item against the
prior work it builds on.

Our main instrument is a linear probe, the simplest possible reader: a
classifier that predicts the trial's answer from a weighted sum of a
representation's numbers, trained on some trials and tested on held-out ones
\citep{alain2016probes}. A probe can succeed because the classifier is
powerful, not because the model uses the information
\citep{hewitt2019control,belinkov2022probing}, so we never rely on a probe
alone. Every probe result is paired with an intervention on the model, in the
tradition of causal mediation analysis \citep{vig2020causal} and activation
patching \citep{meng2022rome,wang2022ioi}, and we report what each
intervention does to the model's own behaviour, not only to the readout
...
present is governed by $A$ and $\Delta$ alone: the multiplication by
$\exp(\Delta_t A)$, a number strictly between zero and one, is the only
operation that removes information, and reading through $C_t$ changes nothing.
Each fact was verified against the installed model source rather than assumed:
$A = -\exp(A_{\log})$ is a per-head scalar, the gate is
$\Delta = \mathrm{softplus}(\Delta_{\text{raw}} + b)$ clamped below at
$0.001$ with no upper clamp, and the decay factor is $\exp(\Delta_t A)$
elementwise.
...
\citep{liu2023llava}, in three question families: which object is present, how
many, and where. Every question is eight-way forced choice, so chance is
exactly $12.5\%$. Trials come in blocks of eight sharing one candidate list,
each candidate correct exactly once per block, which is what makes the
pre-image control meaningful: a classifier that has not seen the image cannot
beat chance. For the splice, images are letterboxed onto a common square
canvas, because caches of different length cannot be exchanged.
...
to uninformative about how many or where. A probe given all 81 layers at once
reads identity at $83.1\%$ ($p = 6 \times 10^{-92}$, $n = 160$), count at only
$19.4\%$ ($p = 0.009$), and position at only $17.5\%$ ($p = 0.041$). The
aggregate probe fits $41{,}472$ features to 120 trials and is pulled down by
overfitting, so we use it only to compare the three properties against each
other; single layers do better. Layer 64 alone reads identity at $92.5\%$, and
the last 21 layers average $85.2\%$ against $59.0\%$ for the middle third of
the stack. The gist is strong, and it is concentrated in the late layers.

\begin{figure}[t]
\centering
\includegraphics[width=0.88\linewidth]{assets/f1_horizon_depth.png}
\caption{\textbf{Left:} identity decoded from the recurrent state against
distance, at three scales, 120 trials per point; the pre-image control reads
exactly $12.5\%$. \textbf{Right:} identity decoded from each layer's state at
the instant of writing; red marks the 13 layers that also carry attention.}
\label{fig:horizon}
\end{figure}
...
sweep of eleven distances (Table~\ref{tab:fine}, appendix) tracks it closely,
the above-chance signal halves at $4.0$ tokens, inside the predicted
interquartile range, and normalised decodability correlates with predicted
mean retention at $r = 0.973$ with no free parameters. Throughout this
collapse the model's own accuracy never moves: $94$ to $98\%$ at every
distance, including where the probe reads near chance.
...
own conclusions. The gate $\bar{\Delta}$ appears in both the decay and the
write term, so it does double duty: it sets how fast a head forgets and how
strongly it writes, by about a factor of seven between the fastest and slowest
thirds, and a head that writes with larger numbers leaves a signal that is
easier to read whatever it contains. Split into naive terciles, the fast third
decodes identity at $61.7\%$ after sixty-four tokens while the slow third
...
One caveat: the strata draw unequal numbers of heads per layer, and depth
matters for decoding, so the comparison \emph{between} strata carries a depth
component. The comparison of each stratum with its own prediction does not,
and each reads far above its own bound.
...
What could be supplying it? Only one component still holds the image at
distance: the attention channel, whose key-value entries do not decay. Each
trial was run twice, identically, except that in one run the image's key-value
entries were evicted from all 13 attention layers immediately after the image
was read, before a single filler token was processed; the recurrent state is
untouched and identical in both runs at that instant. The two hypotheses now
make opposite predictions: if the state remembers on its own, eviction changes
nothing; if it is refreshed from attention, eviction collapses decodability at
distance, worst for the fast heads, which depend most on being refreshed.
...
the fast tercile reads $51.6\%$ intact and $7.8\%$ evicted, below the
$12.5\%$ chance rate; in raw counts, thirty-three correct of sixty-four become
five. The slow heads at distance zero, whose content was written while the
image was still present, barely move ($62.5\%$ to $54.7\%$), showing the
intervention does not simply break the model.

\subsection{Controls: the eviction removes the image, not merely positions}
\label{sec:controls}

A fair objection: the deletion shortens every cache by roughly 400 positions
and changes the softmax normalisation for every later token, so perhaps the
readout collapsed because we damaged the machinery. The control is to delete
the same \emph{number} of positions from the same caches, but text instead of
the image; identical mechanical damage should push both readings the same way.
We ran this on ten strata formed by deciles of predicted half-life, 64 trials
per cell (Table~\ref{tab:controls}, appendix).
...
positions \emph{raises} it by $29.7$ points ($-6.4$ against $+12.5$ averaged
over the ten strata). Deleting text plausibly helps because the text was
competing with the image for the residual stream; the key point is the signs,
because identical mechanical damage cannot push a reading up. The dissociation
holds on the gain-matched strata at three times the trial count ($n = 192$ per
cell): image eviction costs the fast stratum $22.4$ points where matched text
eviction \emph{raises} it $13.5$.

The same collection locates where the relayed information enters. Zeroing the
visual entries at only the three earliest attention layers reproduces $15.8$
of the $17.2$ points lost when all thirteen are zeroed; the three latest cost
$0.5$. Visual information enters early and is consumed downstream. (Per-layer
conditions zero entries rather than delete them; where both operations exist,
at $d{=}32$, they agree within $2.34$ points.)

Finally, the decay parameters predict the size of the dependence: a head that
forgets quickly must depend more heavily on being refreshed, so evicting the
refresh's source should hurt it more. At matched write gain the fast stratum
loses $22.4$ points beside the image against $9.4$ for the slow, a $2.4\times$
dose-response at equal amplitude ($n = 192$ per cell); on the uncontrolled
decile split the drop correlates with the decay rate $1/t_{1/2}$ at $r = 0.800$
($95\%$ CI $[0.34, 0.95]$, $n = 10$) at 32 tokens. The decay calculation
carries predictive content about the mechanism rather than the curve: it
identifies which heads are relaying and which are retaining.

\begin{figure}[t]
\centering
\includegraphics[width=0.88\linewidth]{assets/f3_controls.png}
\caption{\textbf{Left:} against the no-eviction baseline, text eviction sits
above and image eviction below at every distance; both delete the same number
of positions, so damage alone cannot explain opposite signs. \textbf{Right:}
accuracy lost when visual entries are zeroed at all 13 attention layers, the
earliest three, or the latest three: the earliest three carry the effect.}
\label{fig:controls}
\end{figure}

\keyfinding{What a probe reads out of the recurrent state at distance is not
what the state remembered but what the attention channel is still supplying.
Our evidence does not distinguish re-encoding at every token from a single
re-supply shortly after the image, so we claim the dependence, not its period.
The recurrent channel relays the picture; it does not store it.}

\subsection{What the probe can see, and why that matters here}
\label{sec:sensitivity}

A second objection is that the probe may simply miss faint signals: freshly
re-encoded content sits at higher amplitude than a decayed trace, so a probe
insensitive to faint content could produce all of these observations even if
the state never stopped storing the image. This can be measured, and we
measured it. We take the distance-zero features and weaken each trial's own
contribution while substituting unrelated variation from another trial at
constant total variance,
$x_i(\alpha) = \mu + \alpha (x_i - \mu) + \sqrt{1-\alpha^2}\,(x_j - \mu)$,
which mimics decay followed by overwriting. The probe loses half its signal at
$\alpha = 0.78$ and is at chance by $\alpha \approx 0.25$: it cannot see
content below roughly a quarter of full amplitude. The weights say a retained
trace at thirty-two tokens sits at $3.7\%$ of full amplitude at the median
head and $18\%$ at the mean, both below that floor, so a genuinely retained
trace \emph{could not} produce a visible reading. Yet the intact probe reads
the fast tercile at $51.6\%$ there. Whatever it reads must be high in
amplitude, which means recently written; and removing the attention channel's
copy removes it.

\emph{The probe's insensitivity to faint signals is what makes the positive
reading informative: a signal it can see at thirty-two tokens is necessarily
fresh.} The same measurement bounds the claim. Whether faint retained content
persists below the floor is not answerable with this instrument, so we do not
claim the state stops storing the image; we claim that what a probe reads at
distance is not retention, and that it depends causally on the attention
channel.

\begin{figure}[t]
\centering
\includegraphics[width=0.92\linewidth]{assets/f7_sens_splice.png}
\caption{\textbf{Left:} the probe's detection floor. The shaded band is what
decay leaves of a stored trace after 32 tokens; both probes are at chance
throughout it, so a visible reading there cannot be retained content.
\textbf{Right:} how many attention caches must be exchanged before the answer
follows the other image; the leftmost point is the full recurrent swap, with
no effect.}
...
The splice tests the same claim on the model's own behaviour, with no probe
involved. Two runs see different images, one memory channel is exchanged, and
the host run answers its question. A pair is admitted only if host and donor
each answered correctly beforehand, so the ceiling is $100\%$ by construction;
of 111 attempted pairs, 11 were rejected on that criterion, and every
...
Three things follow from the table. Replacing the recurrent memory has no
effect at all: exchanging all 81 states leaves $100$ of $100$ answers on the
host's own image. The attention channel is causally sufficient: seven of
thirteen caches move $94$ of $100$ answers to the donor's. And the dependence
is graded and distributed: one layer changes nothing, seven move almost
everything, and the earliest three matter where the latest three do not,
matching the layerwise eviction of Section~\ref{sec:controls}.

Why does replacing the entire recurrent memory change nothing? The transplanted
state is decayed away within a few tokens and rewritten from the attention
channel, which the swap leaves untouched. The probe result and the splice
result are the same fact seen from two sides.

\section{Scale, and Where the Prediction Fails}
\label{sec:scale}

We repeated the core protocol at three model scales. The instruments read
every tensor shape from the loaded checkpoint's own configuration, which is a
requirement rather than a convenience: the smaller checkpoints differ from the
7B in group count and state dimension.
...
Two conclusions follow, the second a negative result we report plainly. The
parameter-derived horizon is short at every scale: median predicted half-lives
of $6.64$, $2.91$ and $2.58$ tokens, with $85.0$ to $97.9\%$ of heads under 32
tokens. But the predicted horizon does not predict the measured one: across
scales the two move in opposite directions, so the $r = 0.973$ agreement on
the 7B does not replicate and we do not claim the decay calculation explains
the measured curve. Section~\ref{sec:relay} shows why it cannot: the probe at
distance was never measuring retention in the first place.
...
cost against the unpruned baseline is $6.2$, $0.0$, $4.2$ and $6.2$ points,
flat within noise, so at that level the redundancy premise holds. Below it,
distance matters a great deal. At a twentieth retained the cost triples across
the range, from $10.4$ points beside the image to $33.3$ at 512 tokens; with
every visual entry removed it runs from $33.3$ to $75.0$
(Table~\ref{tab:eviction_cost}, appendix). The unpruned row is flat, so long
prompts are not hard in themselves; they become hard once the visual entries
are gone, because the recurrent channel then has nothing left to relay from. A
method validated at short range therefore understates its long-range cost
threefold at $5\%$ retention.

The zero-retention row measures what the recurrent channel contributes on its
own: accuracy falls from $64.6\%$ beside the image to $22.9\%$ at 512 tokens,
a behavioural measurement that agrees with the probe. One discrepancy is worth
stating plainly. At zero retention the model still answers at $37.5\%$ at
thirty-two tokens while the probe reads the state near chance, so the model
uses something the probe does not recover. The probe is a lower bound, and our
evidence does not support the sentence ``the state is empty''.

\section{Limitations}

\textbf{The probe is a lower bound, and our own behavioural data prove it.}
With every visual entry removed, the model answers at $37.5\%$ at thirty-two
tokens while the probe reads near chance, so every statement about what the
state contains is a statement about what is linearly decodable under this
instrument, and accuracies are comparable only within a collection.

\textbf{A sensitivity asymmetry is bounded but not eliminated.} The floor
argument of Section~\ref{sec:sensitivity} assumes decay plus overwriting acts
on the discriminative component roughly as our attenuation does. A model with
no attention channel would settle this without the assumption; we have not
tested one. The route is also located but not traced past the early attention
layers.

\textbf{The dose-response is confirmed at matched amplitude only at distance
zero.} At thirty-two tokens the gain-matched fast stratum reads too close to
chance even when intact for the contrast to be informative. Two strata at one
distance in one model is a direction, not a curve.

\textbf{One model family, and a second family we tried and failed on.} All
results come from three Zamba2-VL checkpoints, the principal limit on
generality. On Granite 4.0 h-tiny \citep{granite4} the horizon calculation
transfers cleanly (median $8.7$ tokens) but the relay contrast does not: the
probe reads that model's state at chance even at distance zero on a task it
answers at $100\%$ (Appendix~\ref{app:granite}), so no relay claim extends
beyond Zamba2-VL on this evidence. The sharper test remains a vision-language
model with no attention at all, such as Cobra \citep{zhao2025cobra} or
VL-Mamba \citep{qiao2024vlmamba}, both Mamba1 models needing the read
operator ported first.
...
it costs generality to free-form answering. $\bar{\Delta}$ is input
dependent: the median half-life is stable across images ($6.44$ to $6.86$
tokens), but a filler with very different gate statistics would give a
different horizon, which the same calculation predicts.

\section{Conclusion}

The recurrent state of this hybrid vision-language model is not a memory of
the image in any useful sense. It is a selective summary of the gist, held for
about four tokens, a lifetime computable from the decay parameters before any
measurement is taken. What the model answers from lives in the thin attention
channel, and the practical consequence is direct: a hybrid's visual key-value
entries are redundant only while the recurrent state is still relaying the
